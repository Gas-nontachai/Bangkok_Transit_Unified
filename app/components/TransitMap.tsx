...
export function TransitMap({
  ...
  for (const sl of stationLines) {
    ...
    if (!line) continue;
    if (!stationLineMap.has(sl.station_id)) stationLineMap.set(sl.station_id, []);
  ...
  useEffect(() => {
    ...
    const drawRoute = (L: unknown, mapInstance: unknown) => {
      ...
      const flushPolyline = () => {
        if (currentPoints.length > 1 && currentLineId) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const polyline = (L as any).polyline(currentPoints, {
            color,
            weight: 5,
            opacity: 0.8,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
